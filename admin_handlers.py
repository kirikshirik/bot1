...
from config import USER_ID_COLUMN, USER_ROLE_COLUMN, SCHEDULER_TIMEZONE, PRODUCTION_SITES, DOWNTIME_REASONS, LINES_SECTIONS
...
async def process_role_choice(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if new_role == "DELETE":
            if cell:
                roles_ws.delete_rows(cell.row)
                action_message = f"Роль для `{target_user_id}` удалена."
            else:
                action_message = f"Пользователь `{target_user_id}` не найден."
        else:
            if cell:
                roles_ws.update_cell(cell.row, 2, new_role)
            else:
                roles_ws.append_row([target_user_id, new_role])
...
# --- НОВЫЙ ОБРАБОТЧИК ДЛЯ КНОПКИ "Статус линий" ---
...
async def show_past_downtime_confirmation(message: types.Message, state: FSMContext):
    ...
    text = [
        ...
        f"**Площадка:** {data['site_name']}",
        f"**Линия/Секция:** {data['ls_name']}",
        f"**Направление:** {data['reason_name']}",
        f"**Начало простоя:** {start_time_str}",
        f"**Окончание простоя:** {end_time_str}",
        f"**Длительность:** {data['duration_minutes']} мин.",
        f"**Описание:** {data['description']}",
        f"**Отв. группа:** {data['responsible_group_name']}\n",
        "Все верно?"
    ]
...
async def save_past_downtime(cb: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        record_data = {
            ...
            "Timestamp_записи": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ...
            "Начало_смены_простоя": shift_start_str,
            "Конец_смены_простоя": shift_end_str,
            ...
            "Кто_завершил_работу_в_группе_ID": "", "Кто_завершил_работу_в_группе_Имя": "",
...
def register_admin_handlers(dp: Dispatcher):
    ...
    # Глобальная отмена для всех FSM в этом файле
